...
const EditProfile = () => {
  ...
  const handleSave = async () => {
    if (!formData.firstName || !formData.lastName) {
      setMessage("Поля Ім'я та Прізвище є обов'язковими.");
      setMessageType("error");
      return;
    }
  ...
  return (
    <div className="edit-profile-container">
      ...
      <div className="button-main-container">
        ...
        <button className="button-main" onClick={handleSave}>Зберегти</button>
        <button className="button-main" onClick={() => navigate(`/profile/${auth.currentUser.uid}`)}>Скасувати</button>
        </div>
      </div>
